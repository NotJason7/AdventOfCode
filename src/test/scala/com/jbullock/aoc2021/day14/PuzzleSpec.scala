...
class PuzzleSpec extends AnyFlatSpec with Matchers {
  val example: List[String] = Source
    .fromResource("aoc/2021/Day14/Example.txt")
    .getLines
    .toList
  ...
  "PolymerLong" must "apply rules correctly" in {
    val start = PolymerLong.fromString(example.head)
    val rules = PairInsertion.fromList(example.drop(2))
    val apply1 = start.applyRules(rules)
    val apply2 = apply1.applyRules(rules)
    val apply3 = apply2.applyRules(rules)
    val apply4 = apply3.applyRules(rules)
    ...
    val apply3MultiLoop = start.loopApply(rules, 3)
    val apply4MultiLoop = start.loopApply(rules, 4)
    val expected1 = PolymerLong.fromString("NCNBCHB")
    val expected2 = PolymerLong.fromString("NBCCNBBBCBHCB")
    val expected3 = PolymerLong.fromString("NBBBCNCCNBBNBNBBCHBHHBCHB")
    val expected4 = PolymerLong.fromString("NBBNBNBBCCNBCNCCNBBNBBNBBBNBBNBBCBHCBHHNHCBBCBHCB")
